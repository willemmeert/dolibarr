 ...
 * Copyright (C) 2024      Willem Meert         <willem.meert@mema.be>
 *
...
/**
 *	\file       htdocs/core/modules/propale/doc/pdf_mmtpropm.modules.php
 *	\ingroup    propale
 *	\brief      File of Class to generate PDF proposal with "MMT propale machine" template
 */
 ...
 */
class pdf_mmtpropmp extends ModelePDFPropales
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "MMTpropMP";
		$this->description = $langs->trans('DocModelMMTPropMPDescription');
	...
	protected function _signature_area(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$pdf->SetFont('', '', $default_font_size - 1);        
        $index = $pdf->MultiCell($largcol, $tab_hl, $outputlangs->transnoentities("ProposalCondMach"), 0, 'L');
        $pdf->Ln();
		$pdf->SetX($posx);
		$pdf->MultiCell($largcol, $tab_hl, $outputlangs->transnoentities("ProposalCondPrice"), 0, 'L');
		...
		$pdf->SetXY($posx, $tab_top + 0);
		$pdf->SetFont('', '', $default_font_size - 2);
		$index = $pdf->MultiCell($largcol, $tab_hl, $outputlangs->transnoentities("ProposalCustomerSignature"), 0, 'L', 1);

		$pdf->SetXY($posx, $tab_top + $tab_hl * $index);
